...
##college <- read.csv("RLab6105/data/College.csv")
...
names(college)  ##returns names of all columns 
str(college)  ##returns datatype of columns


## select a manageable dataset for the following exercises
df <- college[270:299,] ##creates subset of dataset
summary(df)
...
private <- subset(df,Private=="Yes")  ##more preferable
...
## [your code here]
df_new<-filter(df, Grad.Rate >= 80)
df_new<-arrange(df_new, desc(Grad.Rate))
df_new<-select(df_new, X, Grad.Rate)
df_new
...
##DF:
dfx<-select(df, Private, Outstate)
dfx<-group_by(dfx,Private) %>% summarise(max=max(Outstate),
                                      min=min(Outstate))
dfx

##College:
dfx<-select(college, Private, Outstate)
dfx<-group_by(dfx,Private) %>% summarise(max=max(Outstate),
                                         min=min(Outstate))
dfx
## ============================================================================
